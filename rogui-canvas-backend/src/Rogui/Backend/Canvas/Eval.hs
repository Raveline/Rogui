...
import Rogui.Graphics.DSL.Instructions (BlendMode (..), Instruction (..), Instructions, TextAlign (..))
...
convertToSDLBlendMode :: BlendMode -> SDL.BlendMode
convertToSDLBlendMode = \case
  AlphaBlend -> SDL.BlendAlphaBlend
  Add -> SDL.BlendAdditive
  _ -> SDL.BlendNone

-- | Using the given render, default console and default brush, apply a set
-- of instructions.
evalSDLInstructions :: (MonadIO m) => SDL.Renderer -> M.Map Brush SDL.Texture -> Console -> Brush -> Instructions -> m ()
...
eval textures instruction = do
  ...
  case instruction of
    ...
    OverlayAt at colour mode ->
      overlayRect renderer console brush at (V2 1 1) colour (convertToSDLBlendMode <$> mode)
    ...
    FullConsoleOverlay colour mode ->
      overlayRect renderer console brush (V2 0 0) (V2 (width console ./.= tileWidth brush) (height console ./.= tileHeight brush)) colour (convertToSDLBlendMode <$> mode)
...
import Rogui.Graphics.DSL.Instructions (glyphAt, setColours, withBrush)
...
entitiesLayer :: (Foldable f) => Brush -> f entity -> (entity -> GlyphInfo) -> (entity -> V2 Cell) -> MapViewport -> Component m
entitiesLayer brush entities entityDisplayInfo entityPos viewport@(topLeft, _) =
  let drawSingle e = do
        let pos = entityPos e

...
findConsole :: (Ord rc, MonadError (RoguiError err rc rb) m) => rc -> Rogui rc rb n s e r t m' -> m Console
findConsole consoleRef Rogui {..} =
  case consoleRef `M.lookup` consoles of
    Nothing -> throwError (NoSuchConsole consoleRef)
    Just c -> pure c

consoleBelow :: (Ord rc, MonadError (RoguiError err rc rb) m) => rc -> Rogui rc rb n s e r t m' -> m (V2 Pixel)
consoleBelow rc rogui = do
  Console {..} <- findConsole rc rogui
  pure $ position + V2 0 height

consoleRight :: (Ord rc, MonadError (RoguiError err rc rb) m) => rc -> Rogui rc rb n s e r t m' -> m (V2 Pixel)
